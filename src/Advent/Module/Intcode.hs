...
data ICom
  = ICLaunch [Int] (Either (NESeq Int) U.URI)
...
intcodeBot mgr v =
  C
    ...
    , cHelp =
        T.pack $
          [P.s|Launch or interact with an intcode (2019 Days 2 & 5) process; max runtime %d instr. !intcode help for commands|]
            maxFuel
    ...
    , cResp = uncurry $ \user -> \case
        ...
        ICPush nk (i :| is) -> fmap T.pack . liftIO $ do
          ...
          case nk `M.lookup` curr of
            ...
            Just (NeedFuel _) ->
              pure $
                [P.s|Thread for %s out of fuel; use '!intcode refuel %s' to continue|]
                  (unNick nk)
                  (unNick nk)
            Just (NeedInput next) ->
        ...
        ICRefuel nk -> fmap T.pack . liftIO $ do
          ...
          case nk `M.lookup` curr of
            ...
            Just (NeedInput _) ->
              pure $
                [P.s|Thread for %s still has fuel; use '!intcode %s push <input>' to continue|]
                  (unNick nk)
                  (unNick nk)
