...
puzzleLink =
  C
    { cName = "link"
    , cHelp =
        "Get the link to a given puzzle with the puzzle title (!link 23, !link 2017 16).  If bad day or year given, just returns most recent match."
    , cParse = askLink
...
puzzleThread =
  C
    { cName = "thread"
    , cHelp =
        "Get the link to a given puzzle's reddit discussion thread (!thread 23, !thread 2017 16).  If bad day or year given, just returns most recent match."
    , cParse = askLink
    , cResp = \(y, d) ->
        liftIO $
...
capTimeBot =
  C
    { cName = "cap"
    , cHelp =
        "Get information on the global Top 100 cap time for a day (!cap 23, !cap 2017 16).  If bad day or year given, just returns most recent match."
    , cParse = askLink
    , cResp = \(y, d) -> liftIO $ do
        ct <- getCapTime y d
...
prettyTime =
  formatTime defaultTimeLocale "%H:%M:%S EST"
...
challengeCountdown ::
  ...
  Alert m
challengeCountdown evtSet =
  A
    { aTrigger = pure . challengeEvent
    , aResp =
        traverse (addSantaPhrase . T.pack)
...
getCapTime y d = liftIO $ do
  t <- aocServerTime
  putStrLn $
    [P.s|[CAP DETECTION] Getting global leaderboard cap time for %04d %d at %s|]
      y
      (dayInt d)
      (show t)
  mlb <-
...
privateCapped tok lname lbid cap = risingEdgeAlert "private-capped" 5 False trigger response
  where
    trigger y d = liftIO $ do
      t <- aocServerTime
      putStrLn $
        [P.s|[PRIVATE CAP DETECTION] Getting private leaderboard cap time for %04d %d at %s|]
          y
          (dayInt d)
          (show t)
      mlb <-
